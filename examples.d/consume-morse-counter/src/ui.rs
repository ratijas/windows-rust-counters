...
fn draw_tabs(f: &mut Frame<'_>, app: &App, area: Rect) -> io::Result<Rect> {
    let tabs = app
        ...
        .counters
        .iter()
        .map(|s| Line::raw(s.meta.name_value.clone()))
        .collect::<Vec<_>>();
...
fn draw_stat(f: &mut Frame<'_>, app: &App, stat: CounterStats, area: Rect) -> io::Result<()> {
    ...
    if let Some(figure) = app.view.font.convert(&message) {
        ...
        let widget = Paragraph::new(string)
            ...
            .style(
                Style::default()
                    .fg(COLOR_ON_BACKGROUND)
                    .add_modifier(Modifier::BOLD),
            );
        f.render_widget(widget, chunks[2]);
    }

    let dataset_owned: Vec<_> = stat
        .instances
        .iter()
        .map(|instance| {
            let data = instance
        ...
        .collect();
    let dataset_ref: Vec<_> = dataset_owned
        .iter()
        .map(|(name, data, color)| {
            Dataset::default()
                .name(Line::raw(name))
                .marker(if stat.instances.len() <= 4 {
                    Marker::Dot
                } else {
                    Marker::Braille
                })
...
fn draw_placeholder(f: &mut Frame<'_>, area: Rect) -> io::Result<()> {
    let widget = Paragraph::new(vec![]).block(
        Block::default()
            .borders(Borders::ALL)
...
fn trim_lines_to_width(input: &str, alignment: Alignment, area: Rect) -> String {
    ...
    let mut output = input
        .lines()
        .map(|line| trim(line, alignment, area))
        .collect::<Vec<_>>()
        .join("\n");
    if input.ends_with("\n") {
        output.push('\n');
