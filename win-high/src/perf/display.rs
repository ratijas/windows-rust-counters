...
pub unsafe fn get_sample(
    ...
    unsafe {
        ...
        match counter.CounterType {
            PERF_COUNTER_COUNTER | PERF_COUNTER_QUEUELEN_TYPE | PERF_SAMPLE_COUNTER => {
                sample.Data = (data_ptr as *const u32).read() as u64;
                sample.Time = perf_data.PerfTime;
                if PERF_COUNTER_COUNTER == counter.CounterType
                    || PERF_SAMPLE_COUNTER == counter.CounterType
                {
            ...
            PERF_COUNTER_RAWCOUNT | PERF_COUNTER_RAWCOUNT_HEX | PERF_COUNTER_DELTA => {
                sample.Data = (data_ptr as *const u32).read() as u64;
                sample.Time = 0;
            }
            ...
            PERF_AVERAGE_TIMER | PERF_AVERAGE_BULK => {
                sample.Data = (data_ptr as *const u64).read_unaligned();
                // XXX: duplicate fragment from above, except for DWORD vs LONGLONG reads
                base_counter_ptr = base_counter_ptr.offset(1);
                if (counter.CounterType & PERF_COUNTER_BASE) == PERF_COUNTER_BASE {
            ...
            PERF_COUNTER_TEXT | PERF_COUNTER_NODATA | PERF_COUNTER_HISTOGRAM_TYPE => {
                return Err(());
            }
        ...
        if counter_type
            ...
            .contains(CalculationModifiers::MULTI)
        {
            sample.MultiCounterData =
                ((data_ptr as *const u64).offset(1) as *const u32).read_unaligned();
        }
        Ok(sample)
    }
}
...
pub fn display_calculated_value(new: &Sample, old_opt: Option<&Sample>) -> Result<String, String> {
    ...
    if new
        .CounterType
        .calculation_modifiers()
        .contains(CalculationModifiers::DELTA)
        && old_opt.is_none()
    {
    ...
    if let Some(old) = old_opt {
        if new.Time > old.Time {
            return Err(format!(
                "Sample1 ({}) is larger than sample2 ({}).",
                new.Time, old.Time
            ));
        }
    }
    let display = match new.CounterType.into_raw() {
        //(N1 - N0)/((D1 - D0)/F)
        PERF_COUNTER_COUNTER | PERF_SAMPLE_COUNTER | PERF_COUNTER_BULK_COUNT => {
            ...
            let suffix = if new.CounterType.into_raw() == PERF_SAMPLE_COUNTER {
                ""
            } else {
                "/sec"
            };
        ...
        PERF_COUNTER_MULTI_TIMER => {
            ...
            let value =
                100.0 * (numerator as f64 / denominator as f64) / old.MultiCounterData as f64;
            format!("{}%", value)
        }
        // 100*((N1 - N0)/(D1 - D0))/B1
        PERF_100NSEC_MULTI_TIMER => {
        ...
        // 100*(B1- ((N1 - N0)/(D1 - D0)))
        PERF_COUNTER_MULTI_TIMER_INV | PERF_100NSEC_MULTI_TIMER_INV => {
            let old = old_opt.unwrap();
            let numerator = old.Data - new.Data;
            let denominator = old.Time - new.Time;
            let value =
                100.0 * (old.MultiCounterData as f64 - (numerator as f64 / denominator as f64));
            format!("{}%", value)
        }
        // N as decimal
        ...
        // N as hexadecimal
        PERF_COUNTER_RAWCOUNT_HEX | PERF_COUNTER_LARGE_RAWCOUNT_HEX => {
            format!("{:#x}", new.Data)
        }
        // N1 - N0
